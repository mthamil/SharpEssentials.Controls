...
namespace SharpEssentials.Controls.Markup
{
    ...
    public class CommandExtension : MarkupExtension
    {
        ...
        public override object ProvideValue(IServiceProvider serviceProvider)
        {
            ...
            var lambda = System.Linq.Expressions.Expression.Lambda(delegateType, methodCall, methodCall.Arguments.Cast<ParameterExpression>());
            ...
            if (CanExecute != null)
            {
                ...
                var canExecuteParameters = commandParameter == null 
                    ...
                    : new[] { System.Linq.Expressions.Expression.Parameter(commandParameter.ParameterType, commandParameter.Name) };

                var canExecuteExpression = GetMember(dataContext, CanExecute);
                var canExecuteLambda = System.Linq.Expressions.Expression.Lambda(canExecuteDelegateType, canExecuteExpression, canExecuteParameters);
        ...
        private static MethodCallExpression GetMethod(object root, PropertyPath path)
        {
            ...
            System.Linq.Expressions.Expression current = System.Linq.Expressions.Expression.Constant(root);
            foreach (var component in components)
            {
                var property = current.Type.GetProperty(component);
                if (property != null)
                {
                    current = System.Linq.Expressions.Expression.MakeMemberAccess(current, property);
                }
                else
                {
                    var method = current.Type.GetMethod(component);
                    var parameters = method.GetParameters().Select(p => System.Linq.Expressions.Expression.Parameter(p.ParameterType, p.Name));
                    return System.Linq.Expressions.Expression.Call(current, method, parameters);
                }
            }
        ...
        private static System.Linq.Expressions.Expression GetMember(object root, PropertyPath path)
        {
            var components = path.Path.Split('.');
            System.Linq.Expressions.Expression current = System.Linq.Expressions.Expression.Constant(root);
            foreach (var component in components)
